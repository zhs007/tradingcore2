...
const char* JRJExchange::getTypeName() const { return JRJTypeName; }
...
//                                     Money money, Volume& volume, Money&
//                                     price, Money& fee) {
...
// bool TrDB2Exchange::getDataWithTimestamp(const char* assetsName, TimeStamp
// ts,
//                                          CandleData& data) const {
//   auto c = this->m_mgrData.getCandle(this->getMarketName(), assetsName, ts);
//   if (c == NULL) {
//     // LOG(INFO) << "TrDB2CNFundsExchange:getDataWithTimestamp " <<
//     assetsName
//     //           << " " << ts;
...
//   for (auto it = this->m_lstTimeStamp.begin(); it !=
//   this->m_lstTimeStamp.end();
//        ++it) {
//     if (*it >= tsEnd) {
//       break;
//     }
...
// void TrDB2Exchange::foreachCandlesTimeStamp(const tradingpb::Candles*
// candles) {
//   for (auto i = 0; i < candles->candles_size(); ++i) {
//     auto c = candles->candles(i);
...
int JRJExchange::getTradingDays4Year() const {
...
void JRJExchange::regExchange() {
  ExchangeMgr::getSingleton()->regNewExchange(JRJTypeName,
                                              JRJExchange::newExchange);
}
